...
#     tokens = iter(lines)
#     first = next(tokens).split()
#     n_students, n_teams, n_attrs, n_disagree, team_min, team_max = map(int, first)
#     weights = list(map(int, next(tokens).split()))
#     labels = []
#     for _ in range(n_students):
#         labels.append(list(map(int, next(tokens).split())))
#     disagreements = set()
#     for _ in range(n_disagree):
#         a, b = map(int, next(tokens).split())
#         disagreements.add((min(a, b), max(a, b)))
...
# # With swaps of students, implemention is pairwise swaps, --> team sizes balanced.
# # Student s (currently in team t_s) moves to team t_r
# # Student r (currently in team t_r) moves to team t_s
...
#     #def lower_bound_increment

# class AssignmentNeighbourhood:
